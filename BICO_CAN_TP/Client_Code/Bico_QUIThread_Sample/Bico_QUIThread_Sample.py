...
class Bico_QUIThread_Sample(Bico_QUIThread):
    ...
    def MainTask(self):
        ...
        if len(Bico_QUIThread.getThreadHash()) == 1:
            mess_data = Bico_QMessData()
            mess_data.setMess("terminate")
            mess_data.setData("")
...
if __name__ == "__main__":
    ...
    Bico_QUIThread.create(
        Bico_QUIThread_Sample(Bico_QMutexQueue(), 1, Bico_QMutexQueue(), 1, "task_0", os.path.join(current_path, "", "Bico_QUIThread_Sample.qml"))
    )
    Bico_QUIThread.getThreadHash()["task_0"].start()

    Bico_QUIThread.create(
        Bico_QUIThread_Sample(Bico_QMutexQueue(), 1, Bico_QMutexQueue(), 1, "task_1", os.path.join(current_path, "", "Bico_QUIThread_Sample.qml"))
    )
